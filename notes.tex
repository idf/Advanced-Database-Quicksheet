...
$$
\item For $\{\times, \bowtie\}$, it can further drops the $\sigma_{c}T$ to $T$ if the condition does not refer to attributes in $T$.
\begin{align*}
...
Block based 
\begin{enumerate}
\item Input(x) $\triangleq$ part mem := x with block 
\item Output(x) $\triangleq$ block := x with block
\item Read(x,t) $\triangleq$ t := x in block 
\item Write(x,t) $\triangleq$ x in block := t // write need Input(x) since block based access.
\end{enumerate}
